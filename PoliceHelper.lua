...
function getUpdateByUrl(url)
    local n_file, bool = os.getenv('TEMP')..os.time(), false, {}
    downloadUrlToFile(url, n_file, function(id, status)
        if status == 6 then bool = true end
    end)
    while not doesFileExist(n_file) do wait(0) end
    if bool then
        local file = io.open(n_file, 'r')
        for w in file:lines() do
            local vn = w:match('versionnew: (.*)')
			if Version < vn then
				sampAddChatMessage("{3399FF}[Police Helper] {FFFFFF}Вышло новое обновление скрипта!", -1)
				downloadUrlToFile(updatefile, "moonloader\\PoliceHelper.luac")
				script:reload()
			end
        end
        file:close()
...
function CheckUpdate()
    ...
    assert(bool, '\n\n\n\n{3399FF}[Police Helper] {FFFFFF}При закачке обновления, произошла ошибка.\n\n\n')
end
